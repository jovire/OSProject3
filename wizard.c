...
wizard_func(void *wizard_descr)
{
  ...
  while (cube->game_status != 1)
    ...
    if (self->status == 1) {
      ...
      while (cube->game_status == 0 && self->status == 0)
	{
          sem_wait(&cube->control); 
	  printf("Wizard %c%d in room (%d,%d) wants to go to room (%d,%d)\n",
		...
		/* Goes back to the initial state and try again */
                sem_post(&cube->control);
		continue; //new room choice is full
	      }
	    else
	    {
	      break;
      ...
      sem_wait(&cube->control); 
      sem_wait(&newroom->lock); //val must be > 0 because the room was not full 
      switch_rooms(self, oldroom, newroom);
      sem_post(&oldroom->lock); //val must be > 0 because old room is no longer full
      sem_post(&cube->control);
	      ...
	      if (other->status == 0)
		{
		  printf("Wizard %c%d in room (%d,%d) finds active enemy\n",
			 self->team, self->id, newroom->x, newroom->y);
                  ...
                  self->status = fight_wizard(self, other, newroom);
		}
	      else
		{
