...
{
  /* Count the number of frozen wizards
   * on each team. If the number of
   * frozen wizards is equal to its
   * team size then that team lost!
   */
  ...
  if (teamA_frozen == teamA_size) {
     
     return 2;
  } else if (teamB_frozen == teamB_size) {
...
struct wizard *init_wizard(struct cube* cube, char team, int id)
{
  ...
  /* Initially placing wizards requires correctly
   * decreasing the value of the room's semaphore
   * and increasing its occupancy
   */
  ...
  if (cube->rooms[x][y]->wizards[0] == NULL)
    {
      ...
      w->x = x;
      w->y = y;
      cube->rooms[x][y]->occupancy++;
      sem_wait(&cube->rooms[x][y]->lock);
    }
	  ...
	  if (cube->rooms[newx][newy]->wizards[0] == NULL)
	    {
	      ...
	      w->y = newy;
	      initflag = TRUE;
              cube->rooms[newx][newy]->occupancy++;
	      sem_wait(&cube->rooms[newx][newy]->lock);
	    }
...
interface(void *cube_ref)
{
  ...
  //Create threads
  pthread_t wizardsA[cube->teamA_size];
  pthread_t wizardsB[cube->teamB_size];

  for (i = 0; i < cube->teamA_size; i++) {
   pthread_create(&wizardsA[i], NULL, wizard_func, cube->teamA_wizards[i]);
  }
  for (i = 0; i < cube->teamB_size; i++) {
   pthread_create(&wizardsB[i], NULL, wizard_func, cube->teamB_wizards[i]);
  }

  using_history();
  while (1)
    {
      //Check winner on each loop
      if (flag == 0) {
      ...
      if (cube->game_status == 1) {
        for(i = 0; i < cube->teamA_size; i++) {
         // pthread_cleanup_push(kill_wizards, cube->teamA_wizards[i]);
         // pthread_cleanup_pop(0);
          pthread_cancel(wizardsA[i]);
        }
        for(i = 0; i < cube->teamB_size; i++) {
         // pthread_cleanup_push(kill_wizards, cube->teamA_wizards[i]);
         // pthread_cleanup_pop(0);
          pthread_cancel(wizardsB[i]);
        }
      }
      ...
      else if (!strcmp(command, "start"))
	{
	  if (cube->game_status == 1)
	    ...
	    {
	      cube->game_status = 0;
              printf("Enter 'c' for continuous or 's' to step through.\n");
      ...
      else if (cube->game_status == 0 && !strcmp(command, "c")) {
          
          while (check_winner(cube) == 0) {
            sem_post(&cube->mainControl);
          }

         }
      else if (cube->game_status == 0 && !strcmp(command, "s")) {
          sem_post(&cube->mainControl);
          sleep(1); //Cleans up output
        }
...
main(int argc, char** argv)
{
  ...
  for (i = 0; i < cube_size; i++)
      ...
      for (j = 0; j < cube_size; j++)
	{
	  ...
	  /* Fill in */
	sem_init(&room->lock, 0, 2); //max occupancy = 2
	}
  ...
  sem_destroy(&cube->control);
  sem_destroy(&cube->mainControl);
 
  for (i = 0; i < cube->size; i++) {
    for (j = 0; j < cube->size; j++) {
       sem_destroy(&cube->rooms[i][j]->lock); 
       free(cube->rooms[i][j]);
    }
  }
